...
On wider web and desktop windows, an existing thread's composer settles into a single-line resting
state when it loses focus or you scroll the conversation. Model and mode controls remain available
beside the thread context. Focus the composer or start typing to expand it again. New-thread and
narrow layouts keep the full composer.
